...
- **(a)** an existing automated test (in a project test directory, or a `*.test.*` / `*.spec.*`
  ...
  **Strongest form — a test with recorded discriminating evidence:** the slice's
  `Slice <N> red-phase:` line in `.claude/scratchpad.md`, or an equivalent note in the slice record,
  showing the test was observed FAILING before the change and passing after. A test that has only
  ever been seen passing may assert something that was already true; it proves the code is present,
  not that the change did anything. Where such a record exists, cite it in your report. Where the
  record says `NONE`, check that a reason is declared (a characterization test over pre-existing
  behaviour is legitimate) — an **undeclared** `NONE`, or a slice with no red-phase line at all, is
  itself a Level 4 gap: `finding: no discriminating evidence recorded for <slice>`,
  `verifies_with: observe the test failing against the pre-change state, or declare why it cannot`;
