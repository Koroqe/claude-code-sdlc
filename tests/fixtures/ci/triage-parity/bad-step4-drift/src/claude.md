## Autonomous Development Workflow (MANDATORY, fixture)

This file exists only so `scripts/ci/validate-triage-parity.js`'s seeded
fixture tests have something real to check. Never installed, never shipped —
...
**Step 4 — check the quick-tier signal (FR-1.5):** a request not forced to `full` by Step 2 and not satisfying Step 3 is classified `quick` when the estimated file set contains between 1 and 5 files and describes one bounded, already-understood behavior (a bug with a known root cause, a missing validation, a small new utility function, an adjustment to an existing function's or endpoint's behavior) with no new user-facing flow and no new architectural component.
