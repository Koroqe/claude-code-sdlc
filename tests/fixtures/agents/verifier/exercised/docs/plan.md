...
- **Changes:** data layer filters by owner; service validates and maps; route parses the parameter and returns the result; app registers the route; a test calls the route handler directly with a real owner id and asserts on the response body
