<!-- Fixture purpose: trips the "Wave 1 not tracer-only" BLOCKER check
     (FR-5.10(ii)). Identical to golden-plan.md except Slice 3 has been moved
     into Wave 1 alongside the tracer (Slice 1) — Wave 1 now contains a
     non-tracer slice, which must occupy Wave 1 alone. Every other path below
     resolves under tests/fixtures/plan-critic/project/ and the Deliverables
     Checklist carries the CHANGELOG.md entry item, so no other finding
     should fire. -->
...
|------|--------|----------------|-----------|
| 1    | 1, 3   | `tests/fixtures/plan-critic/project/src/routes/widgets.js`, `tests/fixtures/plan-critic/project/src/app.js`, `tests/fixtures/plan-critic/project/src/components/WidgetBadge.jsx` | Independent — no shared files |
| 2    | 2      | `tests/fixtures/plan-critic/project/src/data/widgets.js` | Depends on Wave 1's wiring |
