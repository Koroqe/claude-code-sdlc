...
**FR-1.7 — sensitive paths, union, never replace:** the fixed default list is **always active**, regardless of what a project declares: any path containing `auth`, `payment`, `billing`, `secret`, or `migration` as a path segment (case-insensitive); any path under `.github/workflows/`; `install.sh`; `.claude/settings.json`; `docs/PRD.md`. A project's `.claude/rules/security.md` MAY additionally declare a `## Sensitive Paths` section listing further globs; a path is sensitive for FR-1.3(c) and FR-2.1/FR-2.2(b) when it matches EITHER the fixed default OR a declared entry. A declared section MUST NOT be read as replacing the fixed default and MUST NOT be capable of narrowing or suppressing it — a project that declares a narrow, trivial, or empty section still gets full default protection. `.claude/rules/security.md` is untrusted, project-supplied input feeding a classification decision, which is why FR-1.7 is its own mandatory `security-auditor` pre-review item.
...
- **UC-2-EC2: a trivial declared `## Sensitive Paths` section cannot narrow the fixed default — union semantics proven (FR-1.7, AC-31)** — a project's `.claude/rules/security.md` declares `## Sensitive Paths` with only `docs/marketing/**` listed. A request reading "update the session-expiry check in `src/auth/login.ts`" is evaluated: `src/auth/login.ts` does not match the project's own declared entry (`docs/marketing/**`), but it DOES match the fixed default's `auth` path-segment rule, which per FR-1.7 remains active regardless of what the project declares. Per FR-1.3(c), the estimated file set overlapping this fixed-default-sensitive path forces `full` — identically to what would happen had the project declared no `## Sensitive Paths` section at all. The declared section is additive only; it has no mechanism by which it could have suppressed or narrowed the fixed default, and none is exercised here.

### Data Requirements
- **Input**: the request text; the project's `.claude/rules/security.md` `## Sensitive Paths` section (if declared) unioned with the FR-1.7 fixed default — never substituted for it
...
**Rationale for which gates survive the reduction (FR-4.8):** Gate 2 and Gate 3 review the diff itself and do not depend on any PRD/use-case/QA artifact; Gate 4 is deterministic. Skipping any of the three would make `quick` a synonym for "unreviewed" — keeping them, with the absence carve-outs step 7 below states explicitly, is what stops that.

### Primary Flow (Plan → TDD (Single `no-changelog` Entry) → Reduced Gate Subset With Absence Carve-Outs, No PRD/Use-Cases/QA)
1. Request classified `quick`, reason stated (e.g. `tier: quick — FR-1.5, bounded bug with known root cause, 1 file`).
2. The orchestrator invokes `planner` **exactly once**, under the new Quick-Tier Contract (FR-4.1): a plain feature/fix description in — no PRD section, use-cases file, QA file, or architecture review supplied, since none exist for a `quick`-tier change. `planner` skips its default Process step 1 documentation-existence read entirely for this mode. The returned slice MUST NOT carry a `**Tracer:** yes` marker — quick tier's single slice is exempt from Section 9's tracer-first requirement by design.
3. `planner` — which has no `Write`/`Edit` tool — returns exactly one slice in the standard `Files:`/`Changes:`/`Verify:`/`Done when:` format, with no `**Tracer:** yes` marker.
...
5. The orchestrator runs `/implement-slice` against this one slice, **passing the literal `no-changelog` token** — exactly as `/develop-feature` Phase 2 already does for full-tier slices (FR-4.3). This token is mandatory, not optional: without it, `/implement-slice` Step 6 writes its own standalone changelog entry under one name while `/merge-ready`'s Finalization step separately writes a second entry under the feature's own name for the same unit of work — two entries where Section 5's exactly-once ownership rule requires one, uncaught by the name-keyed idempotency guard because the two entries' names differ.
6. Within that `/implement-slice` run: Pre-flight Check 4 (confirm `docs/qa/*`/`docs/use-cases/*` exist) is skipped entirely because `## Tier:` reads `quick` (FR-4.4, first half). Additionally, Step 2's delegation to `test-writer` states, verbatim, that no `docs/qa/<feature>_test_cases.md` file exists for this change by design and MUST NOT be treated as a missing input — `test-writer` derives its tests from the slice's own `Verify:`/`Done when:` fields instead (FR-4.4, second half). The tracer-gate check, encountering the marker-free single slice under `## Tier: quick`, prints the tier-aware notice `tier: quick, single-slice plan is exempt from the tracer requirement by design` — never the legacy "treating as pre-F3 plan" wording (FR-4.5). `test-writer` writes tests first, the orchestrator implements, `build-runner` verifies, the slice commits per `src/rules/git.md`.
7. After the slice commits, the orchestrator runs `/merge-ready` (FR-4.6) under the Tier Check preamble (FR-4.7): it reads `## Tier: quick`, runs Gate 0 (Git Hygiene), Gate 2 (Code Review), Gate 3 (Security Audit), Gate 4 (Build Verification), and reports Gate 1, 5, 6, 7, 8 as `SKIPPED (tier: quick)` in the output table rather than running or silently omitting them. Gate 2's and Gate 3's own delegation prompts to `code-reviewer`/`security-auditor` state, verbatim, that no `docs/PRD.md` section, `docs/use-cases/*`, or `docs/qa/*` file exists for this change by design and MUST NOT be reported as a finding — superseding, for the duration of this delegation only, `agents/code-reviewer.md`'s existing "Test cases documented in `docs/qa/`" checklist item and any analogous PRD/use-case item either reviewer would otherwise apply. Neither agent's own file is edited by this feature (FR-4.7).
8. The Finalization trigger condition is read as "all gates that were not `SKIPPED` report PASS" — a `SKIPPED` gate neither blocks finalization nor counts as a FAIL. `/merge-ready` writes the single changelog entry (existing Finalization step, reused unmodified) — the ONLY changelog entry for this feature, because step 5's `no-changelog` token suppressed `/implement-slice`'s own write.

**Postconditions**: exactly 1 `Agent` call to `planner` for this feature (mechanically countable in the transcript); `.claude/scratchpad.md` shows `## Tier: quick`, one un-waved slice with no `**Tracer:** yes` marker and no `Wave:` fields; zero files under `docs/PRD.md`/`docs/use-cases/*`/`docs/qa/*` created or modified; `CHANGELOG.md` gains exactly ONE new entry for this feature, not two (AC-24); `/merge-ready`'s gate table shows Gate 1, 5, 6, 7, 8 = `SKIPPED (tier: quick)` and Gates 0, 2, 3, 4 = PASS/FAIL, with neither Gate 2 nor Gate 3 reporting a PRD/use-case/QA-absence finding; `MERGE READY` is reachable with the 4 non-`SKIPPED` gates all PASS.
...
### Error Flows
- **UC-5-E1: a quick-tier-specific Gate 2/3 failure class — the intentional PRD/use-case/QA absence flagged as a finding despite step 7's carve-out (FR-4.7, AC-25)** — this is NOT the same as UC-5's primary flow, and it is NOT "no `quick`-tier-specific behavior": absent step 7's delegation-prompt carve-out (or if that carve-out is itself defective), `code-reviewer` flags its own "Test cases documented in `docs/qa/`" checklist item as missing, and the Auto-Fix Protocol cannot fix it — creating the QA file would violate `quick` tier's own design (FR-1.5/FR-4.1's "none exist for a `quick`-tier change"). Left unhandled, this exhausts the 3-attempt budget and reports `NOT MERGE READY` on a correctly-scoped `quick`-tier change purely because the absence carve-out did not fire. This is exactly the failure class FR-4.7's verbatim delegation-prompt language exists to prevent — the primary flow's step 7 is the fix, not an incidental detail. A genuine, unrelated Gate 2/3 finding (e.g. a real missing input-validation issue in the diff) is handled by the existing Auto-Fix Protocol unchanged (fix, rerun only the failed gate, 3-attempt budget) — that part of the mechanism has no `quick`-tier-specific behavior; only the PRD/use-case/QA-absence class does.
...
- **Output**: one plan slice; one commit; one changelog entry (never two); a `/merge-ready` table in which 4 gates (0, 2, 3, 4) actually run and 5 gates (1, 5, 6, 7, 8) are reported `SKIPPED (tier: quick)`
- **Side Effects**: `.claude/scratchpad.md` write (`## Tier:`, `## Feature:`, `## Plan`); one commit; one changelog entry (via `/merge-ready` Finalization only — never from `/implement-slice` Step 6, which the `no-changelog` token suppresses)
...
### Edge Cases
- **UC-6-EC1: the named enforcement gap (FR-2.7, Risk 1)** — this trigger, and FR-2.2(b)'s equivalent mid-`quick`-run trigger (UC-7-A1), are both enforced entirely by the orchestrator's own compliance with FR-1.8/FR-2.1/FR-2.2; no hook checks `Edit`/`Write` call counts or paths against the declared tier. A `fast`-tier run that silently makes a second-file edit without stating the escalation is not mechanically blocked by anything this feature adds — a named, accepted gap, not a silent one, per Risk 1's own text.
...
**Actor**: orchestrator; `planner` (Quick-Tier Contract, already invoked); `prd-writer`/`ba-analyst`/`architect`/`qa-planner` (now invoked for the expanded scope, via `/bootstrap-feature`, whose Step 7 always writes an explicit `## Tier:` field on every scratchpad init/reinit, FR-2.8)
**Preconditions**: currently operating under `quick` tier (UC-5); mid-TDD-cycle
**Trigger**: while at `quick` tier, EITHER (a) a Rule 4 condition (`src/rules/error-recovery.md` — architectural decision, new dependency, API contract change, schema migration) is encountered during FR-4's TDD cycle, OR (b) an `Edit`/`Write` call about to be made would target a path FR-1.7 marks sensitive, or would bring the count of distinct files touched by this run's own `Edit`/`Write` calls above 3 (FR-2.2) — both existing points: (a) is Rule 4 detection, which already happens in every TDD cycle; (b) is the same per-`Edit`/`Write`-call check FR-2.1 already performs for `fast`→`quick`, now also checked while at `quick`

### Primary Flow (Rule 4 Mid-TDD → Tier Rewritten First, Then Redirect to `full`, Never Stop-and-Ask)
...
2. Per FR-2.2(a), the run does **not** follow Rule 4's default "stop implementation, present to user" behavior — it does not stop and ask.
3. Per FR-2.4(a), the orchestrator states, verbatim, that escalation to `full` is occurring and names the specific FR-2.2 condition that fired: "Escalating from quick to full — this fix requires adding a new npm dependency, a Rule 4 condition."
4. Per FR-2.4(b), the already-committed quick-tier slice commit remains in place, never reverted.
5. Per FR-2.4(c) — **its own mandatory step, performed immediately, before any further gate or agent invocation** — the orchestrator rewrites `.claude/scratchpad.md`'s `## Tier:` field from `quick` to `full`. This is not cosmetic: without it, FR-4.7's Tier Check preamble would read the stale `quick` value and `/merge-ready` would silently run the reduced gate subset on a feature this escalation requires to get all 9 gates, and a context compaction occurring after this point would have `session:start:spine`'s state re-injection re-surface whatever the scratchpad currently says — the pre-escalation `quick` value if this step were skipped or deferred, the corrected `full` value because this step runs first.
6. Per FR-2.4(d), the orchestrator invokes `/bootstrap-feature` for the full, now-larger scope, supplying the already-completed work as context, so `prd-writer`/`ba-analyst`/`architect`/`qa-planner` document it accurately rather than purely prospectively. `/bootstrap-feature` Step 7 (Initialize Scratchpad) writes `## Tier: full` again here — a confirming no-op against the value step 5 already set, per FR-2.8, not a second, independent source of truth.
7. Per FR-2.4(e), `planner` marks the already-satisfied slice DONE with its existing commit hash in the resulting plan — never re-implemented.
8. Per FR-2.4(f), the run proceeds through the remaining slices and an unmodified, full 9-gate `/merge-ready` — indistinguishable at completion from a request classified `full` from the start, because by step 5, `/merge-ready` reads `## Tier: full` exactly as it would have from the start.

**Postconditions (AC-4)**: `.claude/scratchpad.md`'s `## Tier:` field reads `full` immediately upon escalation — before `/bootstrap-feature` is even invoked (step 5 precedes step 6); a context compaction occurring immediately after escalation, followed by `session:start:spine`'s state re-injection, still reports `full`, never re-surfacing the pre-escalation `quick` value; `/bootstrap-feature` then runs for the expanded scope; the resulting plan shows the already-committed quick-tier slice as DONE with its existing commit hash; the run concludes through a full 9-gate `/merge-ready`, verifiable as Gates 1, 5, 6, 7, and 8 actually running and reporting PASS/FAIL — never `SKIPPED (tier: quick)` — confirming the reduced gate subset was not silently applied to the escalated feature.

### Alternative Flows
- **UC-7-A1: the FR-2.2(b) mid-quick-run trigger — sensitive path or `>3`-file sprawl, independent of any Rule 4 condition (AC-23)** — a `quick`-tier run implementing "add a missing validation" (its estimated file set: 1 file) discovers, mid-fix, that the validation actually needs to live in the auth middleware — a path FR-1.7 marks sensitive. Before making the `Edit` call that would touch it, the orchestrator recognizes the FR-2.2(b) condition has fired: no Rule 4 condition is involved at all, yet the run still escalates to `full`, via the identical FR-2.4 mechanics as the primary flow (state the specific condition that fired — here, the sensitive path, not a Rule 4 category — rewrite `## Tier:` to `full` immediately per step 5 before `/bootstrap-feature` runs, then proceed through steps 6–8). This condition exists because FR-1.3(c)/(d)'s mechanical signals are otherwise enforced at classification time only — a `quick`-tier change can sprawl mid-run into exactly the territory FR-1.3 would have forced to `full` had it been visible up front, and FR-1's one-time classification alone has no mechanism to catch that. The file-count half of this same alternative flow (a `quick`-tier run whose touched-file count would rise above 3) escalates identically, citing the file-count trigger by name instead of a path.
- Beyond (a)'s and (b)'s own variety — any of the four Rule 4 examples (new dependency, API contract change, schema migration, module-boundary restructuring), or either of FR-2.2(b)'s two conditions — escalation proceeds identically through steps 2–8 (3–8 for the (b) trigger, which has no Rule 4 default to override in step 2).
...
### Edge Cases
- **UC-7-EC1: Rule 4 (or FR-2.2(b)) hit on the very first slice, before any quick-tier commit exists yet** — FR-2.4(b)/(e)'s "already-committed"/"already-satisfied slice" language is vacuously satisfied (nothing to preserve or mark DONE). The escalation still proceeds through steps 3, 5, 6, and 8; step 7 simply has nothing to mark.
- **UC-7-EC2: the tier-rewrite step is what makes the compaction case safe — proven by its absence being the defect it fixes** — if step 5 were omitted or deferred until after step 6's `/bootstrap-feature` invocation, a compaction landing between the escalation statement (step 3) and the deferred rewrite would leave `.claude/scratchpad.md` reading `## Tier: quick` at the moment `session:start:spine` re-injects state — silently re-surfacing the pre-escalation tier to the post-compaction session, which would then run `/merge-ready`'s reduced gate subset on a feature FR-2.2 explicitly routed to `full`. This is not a hypothetical: it is the specific defect an architecture review identified and FR-2.4(c)/FR-2.8 exist to close, which is why step 5 is ordered before step 6, not merely present somewhere in the flow.

### Data Requirements
- **Input**: the Rule 4 condition detected mid-TDD-cycle, or the FR-2.2(b) sensitive-path/file-count condition detected at an `Edit`/`Write` call boundary
- **Output**: the escalation statement naming the specific condition; the immediate `## Tier: full` rewrite; the `/bootstrap-feature` invocation; the revised plan with the DONE slice
- **Side Effects**: one `.claude/scratchpad.md` write (`## Tier:` quick→full) preceding any other action; new PRD section/use-cases/QA files/architecture review created for the expanded scope; an unmodified full 9-gate `/merge-ready` run
...
### Edge Cases
- **UC-8-EC1: a full-tier plan that legitimately produces exactly 1 slice** — the tracer slice itself resolves the entire scope. This remains structurally different from `quick` tier: the slice still went through `prd-writer`/`ba-analyst`/`architect`/`qa-planner` first, and `/merge-ready` still runs all 9 gates unmodified (never the FR-4.7 reduced subset), even though the slice count happens to match what a `quick`-tier plan would also produce. Slice count alone is never evidence of tier — only the `## Tier:` value set at classification/escalation time is.
- **UC-8-EC2: legacy scratchpad with no `## Tier:` field at all — treated as `full` (NFR-3)** — a scratchpad written before this feature shipped, or by a run whose `/bootstrap-feature` Step 7 predates FR-2.8's "always write an explicit `## Tier:` field" fix, carries no `## Tier:` field. Per NFR-3, every tier-aware check (FR-4.7's gate subset, FR-4.4's Pre-flight Check 4 bypass, and any other tier-aware check this feature adds) treats an absent `## Tier:` identically to `full` — running all 9 gates unmodified, requiring the full documentation-existence Pre-flight Check 4, exactly as before this feature existed. Per FR-2.8, this case grows decreasingly common going forward, since Step 7 now always writes the field on every scratchpad init/reinit — but the fallback itself remains, so a project's un-upgraded scratchpad is still safe to keep operating against after installing this feature's harness code, with no observable behavior change until Phase 0 triage is actually exercised for a new request.
...
3. `skills/sdlc-fast/SKILL.md`'s `allowed-tools` is `Read, Glob, Grep, Edit, Write, Bash, Agent` — **the `Agent` tool IS granted, deliberately.** FR-2.1's mandated escalation from `fast` to `quick` requires invoking `planner`, `test-writer`, and `build-runner` — all `Agent` calls — so withholding `Agent` would make `/sdlc-fast` unable to perform its own required escalation, dead-ending an unattended run in violation of NFR-1(b) the moment a `fast`-tier request under this override turned out to need a second file. The "no subagent" constraint for the ordinary, **non-escalated** `fast` path (FR-3.1) is therefore enforced **by instruction, not by tool grant** — the same model-discipline posture FR-2.7 already accepts for the rest of this feature's tier-boundary enforcement, not a structural guarantee.
4. FR-2's escalation rules still apply once running: this `/sdlc-fast` invocation's edit turns out to touch a second file — the run escalates to `quick` per UC-6, and because `Agent` is granted, it can actually invoke `planner` to do so, rather than being blocked by its own tool grant.

**Postconditions**: the request that UC-2 would classify `full` instead runs the fast-tier flow (UC-4) — zero `docs/PRD.md`/`docs/use-cases/*`/`docs/qa/*` files created, one commit, one changelog entry — unless FR-2's escalation triggers mid-run, in which case UC-6/UC-7 apply exactly as they would for any fast-tier run, now succeeding because `skills/sdlc-fast/SKILL.md`'s `allowed-tools` includes `Agent` (AC-28).
...
- **UC-9-EC2: the tool-grant fix proven against the defect it replaced (FR-6.1, AC-28)** — an `/sdlc-fast` run whose edit touches a second file successfully escalates to `quick` by invoking `planner` via the `Agent` tool, rather than being blocked by its own tool grant. This is the mechanically checkable proof that the constraint is enforced by instruction (step 3) and not by withholding `Agent`: a prior design that denied `Agent` to enforce FR-3.1's "no subagent" rule structurally would, at the exact moment FR-2.1 required escalation, have left the run with no tool capable of invoking `planner` — an unrecoverable dead end for an override skill NFR-2 requires to complete without a human. Verifying `skills/sdlc-fast/SKILL.md`'s `allowed-tools` line includes `Agent`, and that this escalation path actually completes rather than stalling, is the direct test for this edge case.
...
- **Side Effects**: identical to UC-4/UC-5's own side effects; when escalation fires, an `Agent` call to `planner` (and, per UC-5, `test-writer`/`build-runner`) — never blocked by `skills/sdlc-fast/SKILL.md`'s own tool grant
...
**Preconditions**: a local checkout of the repository (`--local` required — refusal otherwise is UC-12); all 14 `agents/*.md` files present at their current `model:` values, each with a well-formed YAML frontmatter fence
**Trigger**: `bash install.sh --local --profile budget` run from the repository root

### Primary Flow (`--profile budget` Rewrites Exactly the 8 Files Whose Target Differs From `quality`)
...
2. **Preflight pass over all 14 files, committing none of them yet (FR-7.2(a)/(b)/(d)):** for each of the 14 `agents/*.md` files, `install.sh` runs an `awk` pass — tracking a frontmatter-fence counter so only a `model: ` line strictly between the file's first and second `---` fence is eligible, never a coincidental `model: ` string elsewhere in the prompt body — computing the rewritten content and staging it to a same-directory temp file (`agents/.${file}.XXXXXX`, never a bare `mktemp` that could land on a different filesystem). Each per-file pass asserts exactly one substitution occurred (FR-7.2(c)); nothing is `mv`-ed into place during this pass.
3. **Commit pass, only after all 14 preflight passes succeed:** `install.sh` `mv`s each validated temp file over its target. Per FR-8.1's `budget` column, 8 of the 14 roles' target value differs from their `quality` baseline (`architect`, `plan-critic`, `planner`: opus→sonnet; `build-runner`, `doc-updater`, `prd-writer`, `refactor-cleaner`, `test-writer`: sonnet→haiku); the other 6 (`security-auditor`, `ba-analyst`, `code-reviewer`, `e2e-runner`, `qa-planner`, `verifier`) have a `budget` target identical to their current `quality` value, so their `awk` pass produces byte-identical output — a substitution still occurs structurally, but the resulting file content does not change, so `git diff` shows nothing for those 6.
4. Per FR-7.4, only the `model:` line changes on any file that IS rewritten to a different value — `name`, `description`, `tools`, `effort:`, and every prompt-body line remain byte-identical before and after, for all 14 files.
5. After all 14 rewrites succeed, `install.sh` writes `.sdlc-model-profile` at the repository root containing exactly one line: `budget` (FR-9.1, FR-9.2 — atomic write via the same temp-file-then-`mv` pattern `write_receipt()` already uses).

**Postconditions (AC-7)**: `git diff --stat` shows exactly **8** files changed, one changed line each — `architect.md`, `plan-critic.md`, `planner.md`, `build-runner.md`, `doc-updater.md`, `prd-writer.md`, `refactor-cleaner.md`, `test-writer.md` — never 14, because 6 of the 14 roles' `budget` value equals their `quality` baseline and an identical-content rewrite leaves no diff; every one of the 14 files' `model:` value (whether it changed or not) equals FR-8.1's `budget` column value for its role; `.sdlc-model-profile` exists and reads exactly `budget`.

### Alternative Flows
- **UC-10-A1: `--profile inherit`** — every agent's `model:` line is rewritten to the literal string `model: inherit` via the identical mechanism; the field is never deleted (FR-7.3, AC-13). `node scripts/ci/validate-agents.js`'s existing `REQUIRED_FIELDS` check continues to pass (`model` present and non-empty) rather than failing on a missing field, because `inherit` is already a member of `VALID_MODEL_ALIASES`. Since every role's `quality` value is `opus` or `sonnet`, never `inherit`, all 14 files change under this profile — `git diff --stat` here does show 14, distinct from `budget`'s 8.
...
- **UC-10-A3: `--dry-run`** — `install.sh --local --profile budget --dry-run` prints all 14 current/target value pairs (including the 6 whose values would not actually change) and exits 0 with no file modified; `git status` afterward is clean (FR-7.5, AC-8).
- **UC-10-A4: the FR-7.6 mandatory spike — whether a running session picks up the rewrite (AC-30)** — before this mechanism ships, the implementation determines, by direct observation, whether a Claude Code session already running against an installed plugin re-reads `agents/*.md` frontmatter live or snapshots agent definitions at plugin load/session start. The finding is recorded in `install.sh`'s own header comment and in a README note stating plainly whether a new session, a `/plugin` reinstall, or neither is required after `install.sh --profile` for the new `model:` values to take effect. If the finding is "snapshotted," a developer who ran `--profile budget` in step 1–5 above and kept their existing session open would observe the receipt (FR-9) and CI (FR-10) both validating the rewritten checkout while their own live session's agents still run at their pre-rewrite `model:` values — the README note is what prevents this from being silently mistaken for a bug in the rewrite mechanism itself.

### Error Flows
None at this UC's level — refusal paths are UC-12.

### Edge Cases
- **UC-10-EC1: the existing no-node/no-jq CI check continues to pass** — because FR-7.2's rewrite idiom is `awk`/shell `case` only, the existing "Assert `install.sh` invokes neither `node` nor `jq`" CI step's grep continues to produce empty output against the modified `install.sh` (AC-11).
- **UC-10-EC2: preflight abort on a malformed file — zero files modified, a different shape from a mid-run interruption (FR-7.2(c)/(d), AC-29)** — a fixture run of `install.sh --local --profile budget` targets a copy of `agents/*.md` where one file (e.g. `agents/architect.md`) has had its `model:` line deleted entirely from its frontmatter. That file's preflight `awk` pass (step 2) finds zero substitutions instead of exactly one, fails its per-file assertion, and `install.sh` aborts **before the commit pass (step 3) begins** — every temp file already created is removed, and `git status` shows **zero** changed files; the exit code is non-zero and names the file missing its `model:` line. **This is a categorically different shape from UC-14-A1's mid-run-interruption scenario**, which the process being killed mid-`mv` can leave partially applied (some of the 14 files rewritten, the receipt correctly withheld because FR-9.2 gates the receipt write on all-14 success). Here, by contrast, the two-phase preflight-then-commit design (FR-7.2(d)) makes a *known-bad input* — not an external interruption — produce an all-or-nothing outcome: the commit pass never starts at all, so there is no "partially applied" state to reach in the first place. Tolerating partial application is correct for an external interruption during the commit pass; it would be wrong for a preflight failure, which is exactly why the two cases use different mechanisms (temp-file survival vs. receipt-gating) to reach the same "no lying receipt" goal.

### Data Requirements
- **Input**: `--profile <name>`; FR-8.1's table encoded as `install.sh`'s own shell `case` arms; each of the 14 files' current frontmatter
- **Output**: up to 14 rewritten `agents/*.md` `model:` lines (8, for `budget`, against the shipped `quality` baseline), one `.sdlc-model-profile` file, OR (on preflight failure) zero file writes plus a named error
- **Side Effects**: up to 14 same-directory temp-file-then-`mv` writes plus 1 new/overwritten receipt file — all only after every one of the 14 preflight assertions passes; never a bare `sed`/`sed -i`, never a partial commit pass following a preflight failure
...
### Alternative Flows
- **UC-14-A1: an interrupted rewrite — during the commit pass, after preflight already passed for all 14 (tolerates partial application)** — every one of the 14 files already passed its individual preflight assertion (UC-10 step 2); the process is then killed mid-commit-pass (UC-10 step 3), after 8 of 14 temp files have been `mv`-ed into place. Per FR-9.2, `.sdlc-model-profile` is written ONLY after all 14 commit-pass `mv`s succeed; an interrupted run therefore leaves NO receipt (or leaves a prior receipt, if one existed, untouched) rather than a receipt claiming a profile 6 files never actually received. A subsequent validator run against this half-rewritten tree either reads no receipt (falls back to `quality`, FR-9.3) or reads a stale prior receipt — in both cases the comparison correctly reports the un-rewritten files as drifted, rather than the receipt lying about what was applied. **This is a categorically different shape from UC-10-EC2's preflight-abort case**: here, every file already passed preflight individually before the interruption — a partially-applied tree is tolerated because the receipt (not the file tree) is what's gated on completeness. UC-10-EC2's preflight failure, by contrast, forbids the commit pass from ever starting, so no file — not even one whose own preflight passed — is written; the "no lying receipt" goal is reached by receipt-gating here, and by never entering the commit pass at all in UC-10-EC2.
...
### Edge Cases
- **UC-16-EC1: quick-tier completion produces no row (AC-16, third clause)** — a quick-tier feature's `/merge-ready` run reports Gate 7 as `SKIPPED (tier: quick)` per FR-4.7 — Gate 7 never runs, so `doc-updater` is never delegated to for a digest write, and no row is produced (FR-12.6). This is the direct consequence of FR-4.7's gate subset, not a separate suppression rule this feature adds.
...
### Alternative Flows
- **UC-17-A1: no active feature, no in-progress `/merge-ready` run (AC-18)** — the status line omits the wave/slice and gates segments entirely — never rendered as a degenerate `0/0` — while still rendering cost and usable-context. This also covers the fast-tier case: wave/slice is omitted when no plan exists (FR-13.3). Note a non-escalated fast run never writes a scratchpad at all (FR-3.5), so `## Tier: fast` is a state that never exists — the omission keys on plan absence, not on a tier read (FR-13.3), since fast tier has no wave/slice concept at all.
...
- **UC-18-EC2: no Node runtime on the machine — a distinct failure shape from Scenario A's "unconfigured" case (FR-13.8)** — `statusLine` IS configured (`.claude/settings.json` names `node .claude/statusline.js`), but the machine has no Node runtime installed. `templates/statusline.js` itself has zero npm runtime dependencies (consistent with every other Node artifact this harness ships), so there is no missing-dependency failure inside the script — but Claude Code's own attempt to spawn `node .claude/statusline.js` fails visibly, once per response, surfaced by Claude Code itself as a command-spawn failure. This is a property of Claude Code's own command-invocation behavior, not code this feature controls or could make silent even if it wanted to. **This is deliberately NOT the same condition as Scenario A** (FR-13.6, `statusLine` unconfigured → nothing renders, no failure signal at all): here a failure signal IS surfaced, on every response, distinguishing "not configured" (silent-by-design absence) from "configured but cannot run" (a visible, per-response spawn error) — the two are recorded as distinct so an implementation is not tempted to treat "no Node runtime" as if it degraded the same way as "no `statusLine` field."

### Data Requirements
- **Input**: (A) none — the absence of configuration itself; (B) the statusline's own stdin JSON, present, plus a corrupted scratchpad; (EC2) a `statusLine` configuration pointing at a `node` binary that does not exist on the machine
- **Output**: (A) no status line at all, no other observable change; (B) a minimal, non-empty status line; (EC2) a visible, per-response command-spawn failure surfaced by Claude Code itself, not a silent absence
- **Side Effects**: none in any scenario
...
| UC-9 (+A1, EC1, EC2) | Explicit Override — `/sdlc-fast`, `/sdlc-quick` | FR-6.1, FR-6.2, FR-6.3, NFR-2 |
| UC-10 (+A1–A4, EC1, EC2) | Model Routing — `install.sh --profile` Rewrite | FR-7.1, FR-7.2, FR-7.3, FR-7.4, FR-7.5, FR-7.6, FR-8.1, FR-8.2, FR-11.1, FR-11.2, FR-11.3, NFR-4, NFR-6 |
