...
5. The candidate's `projectPath` is resolved via `fs.realpathSync` and compared against the current session's `cwd`, also resolved via `fs.realpathSync` (FR-2.2). They resolve to the same real path — a match.
...
### Edge Cases
- **UC-1-EC1: trailing-slash / realpath path matching** — the registry's `projectPath` is recorded as `/Users/dev/Projects/booka/` (trailing slash) or as a path through a symlinked prefix (e.g. macOS `/tmp/x` vs. its real target `/private/tmp/x`), while `cwd` is reported without the trailing slash or through the non-symlinked form. `fs.realpathSync`, applied identically to both sides, resolves the symlink and normalizes the trailing separator, so the two still match (FR-2.2). When `realpathSync` itself fails on either side (the recorded path no longer exists — a rename/move, or a permission error), the fallback is a normalized string comparison stripping a single trailing separator; if that also fails to establish equality, the entry is treated as no match for that entry, never as an error (accepted v1 scope limit, PRD 12.10 Risk 1) — this degrades to UC-1-A1's silent no-line outcome, never to a false positive.
