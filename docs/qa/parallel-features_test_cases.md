...
| TC-4.9 | AC-7 | STATIC | `tests/hooks/test-guards-cross.js`'s split invariant: env-allowlist/`core.fsmonitor` shape asserted at the spawn site, delegation asserted at consumers | `hooks/lib/git-safe.js` (spawn site) and its two consumers (`stop-typecheck-format.js`, `session-start-spine.js`) | Run `tests/hooks/test-guards-cross.js` | The allowlist-env + `core.fsmonitor` structural shape is asserted only inside `hooks/lib/git-safe.js` (path-joined from `REPO_ROOT`, not `HANDLERS`); each of the two consumers is asserted to `require` `git-safe.js` and to contain no direct git child-process spawn (`execFileSync` or `spawnSync`) — literal-shape assertions are never duplicated into the consumer files |
| TC-4.10 | AC-7 | STATIC | `gitBranch`'s spawn is migrated onto the hardened helper (was previously unhardened, no env at all) | `hooks/handlers/session-start-spine.js`'s `gitBranch` call | Grep `session-start-spine.js` for a direct git child-process spawn (`execFileSync` or `spawnSync`) vs. a `git-safe.js` require/delegation | No direct git child-process spawn (`execFileSync` or `spawnSync`) remains in `gitBranch`; it delegates to `hooks/lib/git-safe.js` |
...
| TC-CC.3 | NFR-5 | STATIC | `tests/hooks/test-install-messaging.js` no longer snapshots the de-tracked scratchpad: its GUARDED entry is REMOVED | Fresh CI checkout, where the de-tracked `.claude/scratchpad.md` is absent by construction | Run the test suite locally (file present on disk) and from a fresh CI checkout (file absent) — the CI-checkout run is the absence-arm smoke check | Suite passes in both runs; `.claude/scratchpad.md` no longer appears in the suite's GUARDED list, so no `fs.statSync` snapshot of it exists to throw on absence |
