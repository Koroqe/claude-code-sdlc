...
|---|----------|-----------|---------------|-------|-----------------|
| 1.1.1 | FR-1.1 | `.claude-plugin/plugin.json` exists and sets required fields | Implementation complete | Glob for `.claude-plugin/plugin.json`; read and parse as JSON | File exists, is valid JSON, and sets `name`, `version`, `description`, `author`, `license`, and component path fields for `agents`, `skills`, `hooks` |
| 1.1.2 | FR-1.1 | `plugin.json` component paths point at this repo's directories | `.claude-plugin/plugin.json` exists | Read `agents`, `skills`, `hooks` path fields | Values resolve to `agents/`, `skills/`, `hooks/` at the plugin root (relative to `.claude-plugin/`) |
...
| 2.1.2 | FR-2.1 | Relocated agent content matches `src/agents/*.md` byte-for-byte, except version-string changes | Implementation complete | For each of the 13 files, `diff src/agents/<name>.md agents/<name>.md`, allowing only version-string line differences attributable to FR-6 | Every diff is empty, or the only differing lines are version-string lines |
| 2.1.3 | FR-2.1 | `src/agents/*.md` still exists (source of truth unmoved) | Implementation complete | Glob `src/agents/*.md` | 13 files present, matching the filenames relocated to `agents/*.md` |
...
| 4.1.2 | FR-4.1, FR-4.6 | `owns` section lists exactly the 13 agent paths + `claude.md` + 5 rule paths (19 entries) | `manifests/owned-files.txt` exists | Parse the `owns` block (ignoring `#` comments and blank lines); count non-empty data lines | Exactly 19 entries, matching the 13 agent filenames, `claude.md`, and the 5 rule filenames one-for-one |
| 4.1.3 | FR-4.1, FR-4.9 | `legacy` section lists exactly the 5 retired command paths | `manifests/owned-files.txt` exists | Parse the `legacy` block; count non-empty data lines | Exactly 5 entries: `commands/bootstrap-feature.md`, `commands/develop-feature.md`, `commands/implement-slice.md`, `commands/merge-ready.md`, `commands/context-refresh.md` (relative to `~/.claude`) |
...
| 4.2.2 | FR-4.8 | Receipt's remaining lines exactly match the files that specific install placed (19 entries, no more, no fewer) | Fresh sandbox install completed | Count lines 2+ of the receipt; compare the set against the manifest's `owns` section | Line count is 19; the set of paths is identical to the `owns` section's 19 entries |
...
| # | Use Case | Test Case | Preconditions | Steps | Expected Result |
|---|----------|-----------|---------------|-------|-----------------|
| 5.1.1 | UC-1 Primary Flow | Fresh install on an empty sandbox populates all owned files and writes a receipt | Empty `$SANDBOX/.claude` (or nonexistent) | Run `HOME="$SANDBOX" bash install.sh`; then check `$SANDBOX/.claude/agents/*.md`, `$SANDBOX/.claude/claude.md`, `$SANDBOX/.claude/rules/*.md`, `$SANDBOX/.claude/.sdlc-receipt` | `ls agents/*.md \| wc -l` = 13; `claude.md` present; `ls rules/*.md \| wc -l` = 5; `.sdlc-receipt` exists with 19 data lines (per 4.2.2); exit code 0 |
...
| 5.1.6 | UC-1-EC1 | `~/.claude` does not exist at all — installer creates it | `$SANDBOX/.claude` does not exist | Run `HOME="$SANDBOX" bash install.sh` | Exit code 0; `$SANDBOX/.claude/agents/` and `$SANDBOX/.claude/rules/` exist post-run with expected file counts |
| 5.1.7 | UC-1-EC2 | Re-running `install.sh` immediately with no changes is idempotent | Fresh install completed (5.1.1) | Run `HOME="$SANDBOX" bash install.sh` a second time immediately | Exit code 0 on both runs; file content identical before/after the second run (`diff -r` on `agents/`, `rules/`, `claude.md` empty); a second, distinct timestamped backup directory now exists (`ls "$SANDBOX/.claude" \| grep -c '^backup-'` = 2) |
...
| # | Use Case | Test Case | Preconditions | Steps | Expected Result |
|---|----------|-----------|---------------|-------|-----------------|
| 6.1.1 | UC-2 Primary Flow, FR-4.2–FR-4.5, FR-4.9 | Upgrade overwrites all 13 agents, `claude.md`, and 5 rules; removes all 5 legacy commands; leaves personal files untouched | v3.1 sandbox seeded as above | Run `HOME="$SANDBOX" bash install.sh` | Exit 0; each of the 13 agent files `diff`s empty against `agents/<name>.md`; `claude.md` and each rule file diff empty against `src/`; `$SANDBOX/.claude/commands/` contains none of the 5 legacy files; the 3 personal files are unchanged (checksum match) |
...
| 6.1.4 | UC-2-A1 | An upgrade with hand-edited (`model: fable`) agent copies closes the shadowing gap automatically | v3.1 sandbox seeded with `architect.md`, `planner.md`, `security-auditor.md` containing `model: fable` | Run `HOME="$SANDBOX" bash install.sh`; grep the 3 files for `fable` | Grep returns zero matches; each of the 3 files diffs empty against the repo's `agents/*.md` (cross-ref Section 13) |
| 6.1.5 | UC-2-E2 (regression/defect scenario) | An `owns` entry missing from the manifest is never refreshed | Scratch copy of the manifest with one agent path (e.g. `agents/planner.md`) deliberately removed from `owns`; v3.1 sandbox with a stale `planner.md` | Run `install.sh` against the scratch manifest (via a manifest-path override, or by substituting the scratch manifest into a test checkout) | The omitted file's content remains stale (unchanged, still diverging from `agents/planner.md`); every other of the 13 files is refreshed correctly — demonstrates why 4.1.2's 19-entry completeness check on the *production* manifest is mandatory |
| 6.1.6 | UC-2-E3 | Legacy command surviving an incomplete cleanup shadows the plugin skill undetected | Scratch copy of the manifest with one `legacy` entry (e.g. `commands/develop-feature.md`) deliberately removed; v3.1 sandbox with the 5 legacy command files present | Run `install.sh` against the scratch manifest; then run `claude plugin validate .` | `install.sh` and `claude plugin validate .` both report success (exit 0); `$SANDBOX/.claude/commands/develop-feature.md` still exists on disk and its content differs from `skills/develop-feature/SKILL.md` (`diff` non-empty) — demonstrates the exact silent-shadowing mechanism; the production manifest's 5-entry `legacy` completeness (4.1.3) is what prevents this in the shipped repo |
| 6.1.7 | UC-2-EC1 | Upgrade writes missing agent files fresh when fewer than 13 pre-existed | Sandbox pre-seeded with only 10 of the 13 legacy agent files | Run `HOME="$SANDBOX" bash install.sh` | Exit 0; `ls "$SANDBOX/.claude/agents/"*.md \| grep -Ff <(13 harness names) \| wc -l` = 13 — the 3 previously-missing files are now present and match the repo |
...
**Preconditions for this section:** sandbox with the 13 harness-owned agent files plus the 3 personal files (`brand-guardian.md`, `demo-script-writer.md`, `social-copywriter.md`) in `$SANDBOX/.claude/agents/`, each with a recorded checksum.
...
| 7.1.3 | UC-3-A2 | Fresh install with no personal agents present writes only the 13 harness files | Sandbox with no pre-existing `agents/` content | Run `HOME="$SANDBOX" bash install.sh` | `ls "$SANDBOX/.claude/agents/"*.md \| wc -l` = 13; no unexpected file created |
| 7.1.4 | UC-3-E1 | A stale/incomplete manifest leaves an orphaned harness file, but never puts personal files at risk | Scratch manifest with one harness agent path removed from `owns`; 16-file sandbox | Run `--uninstall` against the scratch manifest | The omitted file survives as an orphan; all 3 personal files are removed-from-scope correctly (never listed either way) and, since `--uninstall` only acts on manifest entries, remain present and byte-identical |
...
| 7.1.6 | UC-3-E3 | A personal file sharing a harness agent's exact filename is indistinguishable by path (documented limitation) | Sandbox with a personal, non-harness-authored `planner.md` (differing content from the repo's `agents/planner.md`) | Run `HOME="$SANDBOX" bash install.sh` (upgrade path) | Post-run, `$SANDBOX/.claude/agents/planner.md` matches the repo's `agents/planner.md`, not the developer's personal content — demonstrates the documented path-only-ownership limitation (mitigated by `/agents` verification, Section 13) |
...
**Preconditions for this section:** sandbox already carrying a full harness install (16 agent files including the 3 personal ones, `claude.md`, 5 rules) unless a test case states otherwise.
...
| 9.1.7 | UC-4-EC3 | `--dry-run` on a machine with no prior install previews a fresh install with no errors | Empty sandbox | Run `HOME="$SANDBOX" bash install.sh --dry-run` | Exit 0; stdout lists the 13 agent paths, `claude.md`, and 5 rule paths as what would be created; no error is raised for the absence of prior state |
...
| # | Use Case | Test Case | Preconditions | Steps | Expected Result |
|---|----------|-----------|---------------|-------|-----------------|
| 10.1.1 | UC-5 Primary Flow (receipt present) | `--uninstall` prefers the receipt for the `owns` footprint and removes `legacy` from the manifest | Sandbox with a valid `.sdlc-receipt` from a prior v4.0-style install, 3 personal files, and the 5 legacy command files present | Run `HOME="$SANDBOX" bash install.sh --uninstall` | Exit 0; `claude.md` absent; rule files absent; `agents/` contains exactly the 3 personal files; `commands/` contains none of the 5 legacy files; `.sdlc-receipt` no longer exists |
...
| 10.1.3 | UC-5-A2 | No receipt present — falls back to the manifest's `owns` section (expected v3.1-upgrade case) | Sandbox with harness footprint present but NO `.sdlc-receipt` file | Run `HOME="$SANDBOX" bash install.sh --uninstall` | Exit 0; same postconditions as 10.1.1 (claude.md/rules/agents/commands all correctly cleaned) — fallback is a fully-supported success path, not a degraded one |
| 10.1.4 | UC-5-A3, AC-11 | `--uninstall` still succeeds from the receipt alone when the manifest is unavailable | Sandbox with a valid `.sdlc-receipt`; the checkout's `manifests/owned-files.txt` renamed to `manifests/owned-files.txt.bak` (or the install.sh invocation pointed at a checkout copy where the manifest is absent) | Run `HOME="$SANDBOX" bash install.sh --uninstall` | Exit 0; the 13 agent paths, `claude.md`, and 5 rule paths (per the receipt) are all removed; stdout includes an explicit note that legacy-section cleanup was skipped because the manifest was unavailable; the 3 personal files remain untouched — directly verifies AC-11 |
...
| 10.1.11 | UC-5-EC1 | `--uninstall` then immediate `bash install.sh` (no `--restore`) produces a valid fresh install | Sandbox post-uninstall (10.1.1) | Run `HOME="$SANDBOX" bash install.sh` | Exit 0; postconditions match Section 5's fresh-install outcome (13 agents, claude.md, 5 rules, new receipt) |
| 10.1.12 | UC-5-EC2 | A receipt-listed file already manually deleted is skipped without error | Sandbox with a valid receipt where one listed agent file was deleted manually beforehand | Run `--uninstall` | Exit 0; no error referencing the missing file; all other receipt-listed files are removed normally |
...
| # | Use Case | Test Case | Preconditions | Steps | Expected Result |
|---|----------|-----------|---------------|-------|-----------------|
| 12.1.1 | UC-7 Primary Flow | Killing `install.sh` mid-copy leaves a recoverable mixed state; re-run converges | Sandbox eligible for an upgrade (v3.1-style seeded state per Section 6) | Launch `HOME="$SANDBOX" bash install.sh &`; send `SIGKILL` after a short, controlled delay (or after N of the 13 agent files are observed refreshed); confirm the process is dead; run `HOME="$SANDBOX" bash install.sh` again to completion | After the kill, `$SANDBOX/.claude` is in a mixed state (some agent files refreshed, some not); after the re-run, `diff -r` against a known-good, uninterrupted reference install is empty; final exit code 0 |
...
| 13.1.2 | UC-8 Primary Flow (Resolution) | Re-running `install.sh` overwrites the 3 stale copies, leaving personal files untouched | State from 13.1.1 | Run `HOME="$SANDBOX" bash install.sh` (or `--uninstall` + fresh install); grep for `fable` again; diff the 3 files against `agents/*.md` | Grep for `fable` returns zero matches; all 3 files diff empty against the repo; the 3 unrelated personal files remain byte-identical |
| 13.1.3 | UC-8-A1 | Only one of three stale — refresh is correct per-agent, not all-or-nothing | Sandbox with only `planner.md` stale (`fable`), `architect.md` and `security-auditor.md` already matching the repo | Run `install.sh`; diff all 3 against the repo | All 3 now match the repo; the two previously-clean files are unchanged before/after (no unnecessary rewrite side effects observed via mtime or checksum, at minimum content is unchanged) |
| 13.1.4 | UC-8-E1 | A manifest missing one stale path leaves it un-refreshed after a "completed" migration | Scratch manifest with `planner.md`'s `owns` entry removed; sandbox with stale `planner.md` | Run `install.sh` against the scratch manifest; then `claude plugin validate .` | `planner.md` still reads `model: fable` post-run; `claude plugin validate .` still exits 0 — demonstrates the concrete failure mode; production manifest completeness (4.1.2) is the actual mitigation |
...
| 13.1.7 | UC-8-EC1 | A same-named personal agent is overwritten as if it were the harness's own copy (documented limitation) | Sandbox with a personal, non-harness-authored `planner.md` (deliberately different content) | Run `install.sh` (upgrade path) | Post-run `planner.md` matches the repo's `agents/planner.md`, not the personal content — confirms the path-only-ownership limitation is real and must be documented, not silently assumed safe (duplicate assertion of 7.1.6, retained here for UC-8 traceability) |
| 13.1.8 | UC-8-EC2 | Uniform refresh of all 13 agents regardless of individual drift state | Sandbox with all 13 legacy agents present in mixed states (some hand-edited, some already identical) | Run `install.sh`; diff all 13 against the repo | All 13 diff empty post-run — refresh is unconditional and uniform, not conditional on detecting drift first |
...
| # | Use Case | Test Case | Preconditions | Steps | Expected Result |
|---|----------|-----------|---------------|-------|-----------------|
| 15.1.1 | UC-10 Primary Flow | Sandbox running only `install.sh` gets the full memory-layer footprint including the mandatory pipeline instruction | Empty sandbox | Run `HOME="$SANDBOX" bash install.sh`; grep `$SANDBOX/.claude/claude.md` for a known marker phrase from `src/claude.md` (e.g. "MANDATORY" or "Autonomous Development Workflow") | 13 agents, `claude.md`, 5 rules present (cross-ref Section 5); the marker phrase is present in the installed `claude.md`, confirming the mandatory-workflow instruction is intact |
...
| 21.1.7 | FR-7.2 | `templates/CLAUDE.md` correctly contains zero bare command references (excluded from the 21-file scope) | `templates/CLAUDE.md` exists | Grep the file for the 5 command name patterns | Zero matches, corroborating its exclusion from the sweep scope |
...
| FR-4.8 (receipt format) | 4.2.1, 4.2.2 |
