...
| TC-FR10-1 | FR-10.1–FR-10.4, AC-13, AC-14 (structural, no independent runtime behavior — mirrors the use-case document's own treatment of FR-10) | STATIC | Agent count reads 15 consistently everywhere it is stated, and the existing floor-based validator needs no change | Implementation complete | `ls agents/*.md \| wc -l`; grep `README.md` (opening description and `## The 15 Agents` heading), `install.sh`'s banner, `.claude-plugin/plugin.json`'s `description`, `.claude-plugin/marketplace.json`'s plugin-entry `description`, all for `15`; confirm `scripts/ci/validate-agents.js`'s `MIN_AGENTS` floor (`13`) still passes unmodified | `ls` returns `15`; all four surfaces read `15`/`## The 15 Agents`; `validate-agents.js` requires no edit and passes against 15 files |
