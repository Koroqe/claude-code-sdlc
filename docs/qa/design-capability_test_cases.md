...
- **Piece B — the agent's judgment (design-reviewer's audit, design-foundation's generation):**
  these require actually invoking `design-reviewer` or `design-foundation` against a committed
  fixture and inspecting the returned output. This is the **FIXTURE** kind, mirroring
  `verification-review-upgrade_test_cases.md`'s use of the same term: not automatable in this
  repository's CI today (no scripted LLM-invocation harness exists here), but specified precisely
  enough — fixture contents named, expected output stated as literal strings/table shapes — that a
  human reviewer or a future eval harness can run it exactly as written.
- **Orchestration across a whole `/merge-ready` or `/bootstrap-feature` run** (gate rendering,
  tier-aware `SKIPPED`/`N/A`, the trust-registry check happening before any command runs) is the
  **BEHAVIORAL** kind — driving the orchestrating skill through a real multi-step run and observing
  the aggregate outcome. Also not automatable in this repository's CI today.
...
| TC ID | UC Scenario | Kind | Test Case | Preconditions | Steps | Expected Result |
|---|---|---|---|---|---|---|
| TC-2.1 | UC-2 Primary Flow, AC-5 | FIXTURE | Full happy path: subject-grounding → token derivation → self-check → write, producing a fully-populated `design.md` | Fixture repository: a package manifest, README naming a concrete product ("a farmers'-market inventory tracker for small vendors"), a handful of route files | Invoke `design-foundation` directly against the fixture | `.claude/rules/design.md` is written with every FR-2.2 section populated (design system source of truth, component library, typography, motion tokens, aesthetic direction, `## Preview`); the accompanying report names at least one subject-grounded, non-generic choice and at least one self-check revision (or explicitly states none was needed and why) |
| TC-2.2 | UC-2-A2 | FIXTURE | Existing token files are extended and named, not replaced by an invented parallel set | Fixture repository already has a `globals.css` with CSS custom properties for color | Invoke `design-foundation` | Generated `design.md`'s "design system source of truth" section names the discovered file path and its naming convention; no second, unrelated token naming scheme is introduced |
| TC-2.3 | UC-2-A3 | FIXTURE | Agent-facing UI populates `## AI Interface Patterns` with vocabulary only, no copied code | Fixture repository contains a chat/streaming UI component | Invoke `design-foundation` | Generated `design.md` includes a populated `## AI Interface Patterns` section naming pattern vocabulary (e.g. streaming text states, tool-call chips); section contains no code block |
| TC-2.4 | UC-2-E1 | — | Cannot infer subject — see UC-8 (TC-8.x) | — | — | Cross-reference only, not duplicated here |
| TC-2.5 | UC-2-EC1, AC-5 | FIXTURE | Greenfield repo, no UI code at all — still produces all required sections | Fixture repository: only a README/product description, no route or component files | Invoke `design-foundation` | `.claude/rules/design.md` is still produced with every required section present; only genuinely-undecided fields (e.g. `## Preview`'s exact route list) are left as explicitly-labeled TODO |
| TC-2.6 | UC-2-EC2 | FIXTURE | Re-run on a project with an existing `design.md` extends/revises, not silently overwrites | Fixture repository already has a `.claude/rules/design.md` from a prior run | Invoke `design-foundation` a second time | Report states what changed relative to the prior version; the file is not replaced with a from-scratch generation that discards prior content with no diff narrative |
...
| TC ID | UC Scenario | Kind | Test Case | Preconditions | Steps | Expected Result |
|---|---|---|---|---|---|---|
| TC-7.1 | UC-7 Primary Flow | FIXTURE | Existing token/style files are documented as source of truth, not proposed anew | Fixture repository with an existing Tailwind theme config and an existing component library | Invoke `design-foundation` directly | Generated `design.md`'s design-system-source-of-truth and component-library sections reference the discovered existing files/library by name, not an invented replacement |
| TC-7.2 | UC-7 Primary Flow (slop detection) | FIXTURE | Existing UI exhibiting generic "slop" (unmodified default indigo theme) is flagged in the report, not silently accepted, and no application code is edited | Fixture repository whose existing theme uses an unmodified `bg-indigo-500`-class default palette | Invoke `design-foundation` | Report explicitly flags the existing palette as a self-check candidate revision; `.claude/rules/design.md` is the only file written — no application/component file is modified |
| TC-7.3 | UC-7-A1 | FIXTURE | Report distinguishes "documented as-is" from "flagged for change," leaving the decision to the developer | Same fixture as TC-7.2 | Invoke `design-foundation`; inspect the report's structure | Report has a clear separation between what is documented as the current state and what is flagged as a self-check recommendation — the two are not merged into one undifferentiated list |
...
| TC ID | UC Scenario | Kind | Test Case | Preconditions | Steps | Expected Result |
|---|---|---|---|---|---|---|
| TC-8.1 | UC-8 Primary Flow | FIXTURE | Only the specific un-inferable questions are asked, not a blanket interview | Fixture repository: a bare scaffold with a package manifest revealing the tech stack and route shape ("dashboard-shaped app"), but no README or product copy revealing purpose/audience | Invoke `design-foundation` (interactive path) | The `AskUserQuestion` prompts asked are scoped to what genuinely cannot be inferred (product purpose, audience) — no question is asked about facts already inferable from the manifest/routes (e.g. tech stack) |
| TC-8.2 | UC-8-E1, FR-3.2 | FIXTURE | Developer declines / nothing to ground on → labeled TODO placeholders, no fabricated confidence | Fixture repository with zero product signal (empty README, generic scaffold routes); simulated non-response to `AskUserQuestion` prompts | Invoke `design-foundation` | `.claude/rules/design.md` is still produced; the aesthetic-direction section (and any other genuinely unresolved section) is explicitly marked TODO rather than populated with an invented subject; the report states which sections remain TODO and why |
| TC-8.3 | FR-3.2 (non-blocking clause) | FIXTURE | Unattended `/bootstrap-feature`-triggered invocation never stalls on `AskUserQuestion` | Same low-signal fixture as TC-8.2, invoked via the `/bootstrap-feature` trigger path (unattended) rather than standalone | Invoke `design-foundation` in the unattended/triggered mode | The skill completes without waiting on an interactive answer; it degrades to best-effort inference and TODO placeholders instead of stalling; `.claude/rules/design.md` is produced |
...
| TC-8.6 | UC-8-E1 (negative/false-positive framing) | FIXTURE | A repository with SOME signal is not treated as "cannot infer" — the E1 path is not over-triggered | Fixture repository with a clear README naming the product and audience | Invoke `design-foundation` | No TODO-placeholder fallback is used for sections the README made inferable; `AskUserQuestion` is not asked for facts already stated in the README |
...
|---|---|---|
| STATIC | 66 | Yes — 66/66. Runnable via `node scripts/ci/validate-*.js`, `node tests/hooks/test-stop-gate-evidence.js`, and equivalent grep/file-read/git-diff checks, with zero agent invocations (TC-S.33 is the one shell-level integration exception noted as BEHAVIORAL but scriptable) |
| FIXTURE | 47 | No — 0/47 today. Each requires a live, single-agent invocation (`design-reviewer` or `design-foundation`) against a committed fixture; this repo has no LLM-invocation harness to script that. Fixture contents and expected outputs are specified precisely enough for a human reviewer or a future eval harness to execute exactly as written |
| BEHAVIORAL | 15 | No — 0/15 today (except TC-S.33, a plain Bash-scriptable `install.sh --init-project` integration check, which IS automatable and is the one BEHAVIORAL case that could run in CI as ordinary shell testing). The remaining 14 require driving `/merge-ready` or `/bootstrap-feature` through a real multi-step, multi-agent run and observing the aggregate outcome |
| **Total** | **128** | **67/128 (≈52.3%) automatable in CI today** |
...
of — not instead of — the judgment-dependent agent behavior (`design-reviewer`'s audit,
`design-foundation`'s generation) that remains FIXTURE/BEHAVIORAL for the same structural reason
the prior feature's agent-judgment checks did: there is no LLM-invocation harness in this repository
today. Rounding any FIXTURE or BEHAVIORAL case up to "automated" would misrepresent that ceiling.
