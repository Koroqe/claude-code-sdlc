...
| 15 | Parallel Feature Sessions | Makes parallel per-feature git-worktree sessions a supported mode: de-tracks the scratchpad so it never merges; adds union-merge plus a five-class repair layer for instincts and the changelog; gives Gate 0 a real base-sync procedure (merge, never rebase) with an injection-guarded ref check; allocates PRD section numbers and dual-keys digest rows to avoid collisions; derives release versions from origin, not branch memory; and lets worktrees inherit trust via a hardened shared git helper. No new agents, hooks, or skills. | [PRD §15](PRD.md#15-parallel-feature-sessions) · [use cases](use-cases/parallel-features_use_cases.md) · [QA](qa/parallel-features_test_cases.md) |
| 16 | Run to Completion | An approved plan runs to the end unattended: `stop:gate-evidence` refuses a mid-plan stop with no recorded blocker, bounded by progress; Rule 4 exempts planned changes; partial wave failure gets a policy; legacy stop-after-each-slice files are flagged. No new hook id; version 4.11.0. | [PRD §16](PRD.md#16-run-to-completion) · [use cases](use-cases/run-to-completion_use_cases.md) · [QA](qa/run-to-completion_test_cases.md) |
