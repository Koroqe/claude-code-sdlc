...
5. **FR-5.5 (max 3 revision loops, then escalate):** The critique-and-fix cycle MUST run as follows: (1) invoke `plan-critic` (loop 1); (2) if it returns any BLOCKER finding, fix the plan file for every BLOCKER and WARNING finding, then re-invoke `plan-critic` against the revised plan (loop 2); (3) repeat once more if needed (loop 3). If zero BLOCKER findings remain after any loop, proceed immediately (WARNING findings remaining are acceptable and recorded, per FR-5.6, without a further loop). If a BLOCKER finding remains after loop 3, the workflow MUST escalate per Section 1 FR-2.4 (Rule 4 — present the remaining BLOCKER findings verbatim, the decision needed, and options) rather than proceeding — this applies identically whether the loop was invoked from the plan-mode stub (FR-5.6) or from `/bootstrap-feature` Step 5 (FR-5.9).
...
11. **FR-5.11 (the agent count must read 14 everywhere it is hardcoded, not just `README.md`):** Every live location that states the agent count as 13 MUST be updated to 14 alongside `agents/plan-critic.md` landing: `README.md`'s opening description (line 5, "13 specialized AI agents") and its "## The 13 Agents" heading (line 110); `install.sh`'s install banner (line 123, `agents/  13 specialized agents — ship in the plugin`); `.claude-plugin/plugin.json`'s `description` field (line 4, "...13 specialized agents..."); and `.claude-plugin/marketplace.json`'s plugin-entry `description` field (line 14, the same string). **Named exception, MUST NOT be touched:** `README.md` line 280 — "upgrading from v3.x removes the 13 agent files and 5 command files that older versions installed into `~/.claude/`" — is a historical fact about what v3.x shipped, not a live count, and stays `13` regardless of this feature's agent count.
...
5. **FR-6.5 (skip unchanged code):** For a given `git diff`, `code-reviewer` and `security-auditor` MUST NOT report a finding in code outside the diff's changed hunks, unless that finding is CRITICAL severity (FR-6.6).
