...
**Design Decisions:**
1. **Every guard names its backstop, as a numbered requirement, not a description.** Section 7 FR-3.7 states that fail-open governs mechanism failure only, and that a blocking guard may tolerably fail open exactly when its invariant has a named downstream `/merge-ready` gate that re-checks it. This feature makes "name the backstop" a literal per-guard requirement (FR-2.9, FR-3.7, FR-4.9, FR-5.8, FR-6.7, FR-7.6, FR-8.7 below) rather than leaving it to prose, so a reviewer can confirm the mechanism-failure gap is actually covered.
2. **`pre:edit:read-guard` and `pre:edit:gateguard` each need session-scoped state, so both reuse Section 7 FR-6's accumulator pattern rather than inventing a second one.** Section 7's `hooks/lib/accumulator.js` already solves "session-keyed, sanitized, append-only, gitignored, garbage-collected record under `.claude/tmp/`" for edited paths; this feature adds two more record types (files read this session; files already fact-forced this slice) using the same directory, the same `session_id` sanitization rule (Section 7 FR-6.2), and the same symlink defense (`pathIsSafe`) — not a third bespoke mechanism.
...
7. **`pre:edit:gateguard` ships opt-in, with promotion criteria fixed in advance and a review date, because its cost/benefit is unmeasured.** Unlike the other six guards — which mechanize rules already known to matter — GateGuard's fact-forcing pattern is a hypothesis borrowed from a surveyed harness ("the investigation itself creates context that changes the output"). FR-8.6 makes the promotion decision falsifiable rather than a permanent default-off with no review date, per the autonomy contract's ban on optional-forever mechanisms.
...
3. **FR-5.3 (`@ts-nocheck` and blanket `eslint-disable`, any source file):** Independent of FR-5.2's file-path match, the guard MUST deny an `Edit`/`Write` whose post-edit content introduces a `@ts-nocheck` directive, or a bare `eslint-disable` directive with no specific rule names listed (`/* eslint-disable */` or `// eslint-disable`, as distinct from a rule-scoped `eslint-disable-next-line no-unused-vars`), that was not already present in the file's pre-edit content.
...
6. **FR-6.6:** If FR-6.3's branch is implemented instead, this feature's acceptance criteria for `pre:agent:isolation-guard` (AC-10) are satisfied by demonstrating the visible-warning behavior, not the blocking behavior — the guard's PRD requirement is met by failing visibly, per FR-6.3, not by a blocking capability the spike found unreachable.
...
4. **FR-7.4 (what "blocking" means for a `Stop` hook):** The changelog write this guard inspects has already completed by the time `Stop` fires — there is no tool call left to deny. When any FR-7.2 assertion fails, the guard MUST return a blocking decision for the `Stop` event itself (exit code 2 with the reason on stderr, or JSON stdout with `decision: "block"` and a `reason` string), which causes Claude Code to force another turn instead of ending the response, rather than attempting to prevent a `Write`/`Edit` that has already happened.
...
#### FR-8: `pre:edit:gateguard`

`PreToolUse`, matcher `Edit|Write`. Fact-forcing, opt-in, with promotion criteria fixed in advance.

1. **FR-8.1:** When the environment variable `SDLC_GATEGUARD` is exactly the literal string `on` (per the roadmap's literal-token flag rule — any other value, or unset, means fully off), the guard MUST deny the first `Edit`/`Write` call this slice makes against a given target file, and MUST allow every subsequent call against the same (file, slice) pair without re-prompting.
2. **FR-8.2 ("this slice," state location):** The guard MUST identify the currently active slice from `.claude/scratchpad.md` using the same structured-field extraction Section 7 FR-5.1/FR-5.8 already performs for `session:start:spine` (not raw prose pass-through), and MUST key its "already fact-forced" record by the tuple (sanitized `session_id`, slice identifier, target file path), stored under `.claude/tmp/` using the same accumulator pattern as FR-4.2 (illustratively `hooks/lib/gateguard-state.js`, reusing `hooks/lib/accumulator.js`'s helpers).
3. **FR-8.3 (the fact-forcing content):** The deny reason on the first call against a (file, slice) pair MUST require the model to state, before retrying: (a) which files import or reference the target file — a concrete list, or an explicit "none found" after a search; (b) what public surface (exports, routes, props, schema) the edit changes; (c) a verbatim quote of the current planner instruction (the slice's `Changes:` field, Section 1 FR-3.2) driving this edit.
4. **FR-8.4:** The guard MUST NOT re-deny the same (file, slice) pair a second time regardless of the content of the model's restated answer — the guard forces investigation, it does not grade the answer (per the roadmap's borrowed rationale: "the investigation itself creates context that changes the output").
5. **FR-8.5 (opt-in default, literal-token rule):** With `SDLC_GATEGUARD` unset or set to any value other than the literal string `on`, the guard's handler MUST behave as if absent — no `systemMessage`, no state file write, no denial, for any call. This is Section 7's profile system's precedent (FR-1.5 above) applied to a handler-internal opt-in flag rather than a runtime-control env var Section 7 already governs.
6. **FR-8.6 (promotion criteria, fixed in advance, falsifiable, with a review date):** (a) GateGuard runs with `SDLC_GATEGUARD=on` for 10 consecutive slices, in any project; (b) promotion to a default-on member of FR-1.5's standard/strict membership happens only if, across those 10 slices, `merge-ready`'s first-pass gate-failure rate is lower than the same project's prior 10-slice baseline, AND no single slice was blocked by GateGuard for more than 2 model turns before proceeding; (c) the criteria are reviewed at the roadmap's F5 kickoff; (d) if the 10-slice measurement has not been recorded by F5 kickoff, GateGuard MUST be removed from the codebase rather than left permanently opt-in-and-unmeasured.
7. **FR-8.7 (backstop):** `merge-ready` **Gate 2 (Code Review)** and **Gate 6 (Goal-Backward Verification)** are the backstops — since GateGuard is opt-in and may be off for any given slice, these two downstream checks are what catch an edit made without adequate investigation whether or not GateGuard ran for that slice.
8. **FR-8.8 (escape):** `SDLC_DISABLED_HOOKS` containing `pre:edit:gateguard` disables the guard entirely — a redundant kill switch alongside the opt-in default (FR-8.5), for a project that turned GateGuard on and hit friction it wants to stop without unsetting `SDLC_GATEGUARD` project-wide.

#### FR-9: Cross-Guard Requirements

Requirements that apply identically to all seven guards in FR-2 through FR-8.

1. **FR-9.1 (blocking mechanism, `PreToolUse`):** `pre:bash:git-guard`, `pre:write:shrink-guard`, `pre:edit:read-guard`, `pre:edit:config-protection`, `pre:agent:isolation-guard` (FR-6.2 branch), and `pre:edit:gateguard` MUST block using one of Claude Code's two documented `PreToolUse` mechanisms: (a) exit code `2` with the deny reason written to stderr, or (b) exit code `0` with JSON stdout containing `hookSpecificOutput.permissionDecision: "deny"` and a `permissionDecisionReason` string. No guard MUST invent a third signaling shape.
2. **FR-9.2 (blocking mechanism, `Stop`):** `stop:changelog-guard` MUST block using the `Stop`-event equivalent: exit code `2` with the reason on stderr, or exit code `0` with JSON stdout containing `decision: "block"` and a `reason` string (FR-7.4).
3. **FR-9.3:** Every guard id in FR-2 through FR-8 MUST be listed as a valid target for `SDLC_DISABLED_HOOKS` (Section 7 FR-4.3 applies to these 7 ids without modification — no new mechanism is required, only the `HOOKS` map entries in `hooks/lib/run-hook.js` per FR-1.5).
4. **FR-9.4 (one scoped escape sentinel per guard):** Each guard that can deny a call MUST additionally support its own environment-variable escape, checked inside the handler (not the wrapper), distinct from `SDLC_DISABLED_HOOKS`, so a single legitimate exception does not require disabling the guard's other checks for the rest of the session: `SDLC_ALLOW_GIT_GUARD=1` (FR-2.10), `SDLC_ALLOW_SHRINK=1` (FR-3.5), `SDLC_ALLOW_UNREAD_EDIT=1` (FR-4.6), `SDLC_ALLOW_CONFIG_EDIT=1` (FR-5.6), `SDLC_ALLOW_SUBAGENT_WRITE=1` (FR-6.5), `SDLC_ALLOW_CHANGELOG_SHAPE=1` (FR-7.7). `pre:edit:gateguard` is the sole exception: its self-clearing design (FR-8.4 — a (file, slice) pair is never denied twice) plus `SDLC_DISABLED_HOOKS` already satisfies "never permanently wedge a run" without a redundant third mechanism (FR-8.8).
5. **FR-9.5 (concrete remedy, not a bare restatement):** Every `deny` reason and every `Stop`-blocking `reason` this feature's guards return MUST state the specific next action to take (e.g., "switch to a feature branch: `git checkout -b feat/<slug>`", "set `SDLC_ALLOW_CONFIG_EDIT=1` if this slice legitimately changes eslint config") — never merely restate which rule was violated.
6. **FR-9.6 (deviation-rule classification):** Every guard's deny/block reason MUST be phrased so `src/rules/error-recovery.md`'s four deviation rules (Section 1 FR-2) can classify the failure without ambiguity: a self-correctable rule violation (wrong branch, missing `type(scope):`, an unread file) is a **Rule 1/2** case — fix and retry, free; a guard requiring an explicit environment escape (`config-protection`, `shrink-guard`, `git-guard`'s push check) is a **Rule 3** case — the escape is the resolution, and it costs one retry; a guard whose repeated denial reveals a genuine scope question (e.g., a `GateGuard` investigation showing the edit truly needs a public-surface change beyond the slice's stated scope) escalates under **Rule 4**.
...
#### FR-10: CI Regression Assertion Narrowing

Section 7's own regression guard (its FR-3.5, implemented as the static source sweep in `tests/hooks/test-wrapper.js`) currently asserts no file anywhere under `hooks/` contains an `exit(2)` path or a `permissionDecision` — a sweep this feature's own guards would fail by design. Deleting that assertion would remove Section 7's non-blocking guarantee entirely; this FR narrows it instead.

1. **FR-10.1:** `tests/hooks/test-wrapper.js`'s static source sweep MUST be narrowed to scan only the 4 files Section 7 shipped (`hooks/lib/run-hook.js` and its 3 handlers: `session-start-spine.js`, `post-edit-accumulate.js`, `stop-typecheck-format.js`) rather than the entire `hooks/` tree. The narrowing MUST be a change to the sweep's file list, not a deletion of the assertion — Section 7's non-blocking guarantee for those 4 files remains a real, CI-enforced invariant after this feature ships.
2. **FR-10.2 (positive control):** A new, separate assertion MUST confirm the opposite for this feature's 7 guard handlers: each of `pre-bash-git-guard.js`, `pre-write-shrink-guard.js`, `pre-edit-read-guard.js`, `pre-edit-config-protection.js`, `pre-agent-isolation-guard.js`, `stop-changelog-guard.js`, and `pre-edit-gateguard.js` DOES contain at least one `deny`-capable code path (`permissionDecision: "deny"`, `decision: "block"`, or `process.exit(2)`/`process.exitCode = 2`) somewhere in its source — proving each guard is mechanically capable of blocking, not merely documented as if it could be.
3. **FR-10.3:** Both FR-10.1's narrowed sweep and FR-10.2's positive-control check MUST run in the same CI job (`validate-assets`) that already runs Section 7's hook fixture tests, so a regression in either direction — a Section 7 hook gaining a deny path, or a Section 8 guard losing one — fails CI.
...
1. **NFR-1 (zero dependencies, inherited unchanged):** All 7 guard handler modules MUST have zero npm runtime dependencies, consistent with Section 7 NFR-1 — no `npm install` step is required for any guard to run. Subprocess use (e.g., `pre:bash:git-guard`'s `git rev-parse`, `stop:changelog-guard`'s `git status --porcelain`) MUST use Node's built-in `child_process` module only.
2. **NFR-2 (ES5 syntax floor — scope restated, not re-imposed):** Section 7 FR-2.6 established that `hooks/lib/run-hook.js` and any module it `require`s on the path executed *before* its own Node-version gate MUST parse under the oldest plausible Node an adopter's machine might resolve. This feature adds 7 entries to `run-hook.js`'s `HOOKS` id-to-handler map — already ES5-safe object literals — and requires no new module into that pre-gate path. All 7 guard handler modules, like Section 7's 3, load *after* the version gate and MAY use modern syntax freely; this NFR clarifies that the floor's scope is unchanged by this feature, not that this feature re-establishes it.
3. **NFR-3 (backward compatible, no autonomy regression):** A project without the plugin installed, run with `SDLC_HOOKS_ENABLED=0`, or run with all 7 guard ids listed in `SDLC_DISABLED_HOOKS`, behaves identically to a Section-7-only installation with no guards configured — no observable behavior change beyond the guards themselves. Every guard is additive and independently disableable; none of Section 7's 3 hooks changes behavior as a result of this feature.
4. **NFR-4 (asset budget):** This feature adds 7 hooks (the ids in FR-2 through FR-8) to Section 7's 3, for a running total of **10 hooks** against the v4.0 hard budget of **≤12 hooks** (Section 6 NFR-5). Agent count remains 13, skill count remains 5 — unchanged from Section 6/7. This feature alone consumes 7 of the remaining 9 hook slots under the cap; only 2 remain for the roadmap's F3/F4/F5 (see 8.10 Risk 8).
...
9. **AC-9:** An `Edit` to `tsconfig.json` flipping `"strict": true` to `"strict": false` is denied by `pre:edit:config-protection`; the identical `Edit` is allowed when `SDLC_ALLOW_CONFIG_EDIT=1` is set. An `Edit` inserting `@ts-nocheck` at the top of a source file that did not previously contain it is denied under the same escape.
10. **AC-10:** If the FR-6.1 spike confirms a reliable subagent indicator: a `Write` to `.claude/scratchpad.md` or `CHANGELOG.md` made from a parallel-wave subagent context is denied by `pre:agent:isolation-guard`; the identical `Write` from the orchestrator's own context is not denied. If the spike finds no reliable indicator: every such `Write`, from any context, produces the FR-6.3 visible warning, unconditionally, every time — verifiable by a fixture asserting the warning fires on both a simulated subagent call and a simulated orchestrator call identically.
11. **AC-11:** A response in which `CHANGELOG.md` was modified but the new entry is missing a `**Details:**` line, exceeds 500 characters in Details, or duplicates an existing entry name under today's date causes `stop:changelog-guard` to return a blocking `Stop` decision with a `systemMessage`/`reason` identifying the specific defect; an entry meeting all shape requirements does not block `Stop`, regardless of elapsed wall-clock time since the entry's stated `HH:MM UTC`. A fixture asserting 3 consecutive failing checks in one session confirms the guard stops blocking after 2 (FR-7.5) and instead emits a non-blocking warning on the third.
12. **AC-12:** With `SDLC_GATEGUARD=on`, the first `Edit` to a given file within a slice is denied once with a message containing all three fact-forcing elements (FR-8.3); a second `Edit` to the same file within the same slice is not denied. With `SDLC_GATEGUARD` unset (or set to any value other than the literal string `on`), no `Edit` is ever denied or annotated by this guard — verifiable by asserting zero output for this hook id across a fixture run.
13. **AC-13:** The FR-10.2 positive-control check exits non-zero if any of the 7 guard handler modules is found to contain no `deny`-capable code path, and exits `0` against the real, shipped handlers.
14. **AC-14:** The FR-10.1 narrowed static-source sweep continues to exit `0` when scanning only Section 7's 4 files, and its file list, inspected directly in `tests/hooks/test-wrapper.js`, does not include any of the 7 new guard handlers.
15. **AC-15:** `SDLC_DISABLED_HOOKS` set to a single comma-separated value containing all 7 of this feature's ids results in zero denials and zero guard-originated `systemMessage`s across a full pass through every guarded tool-call shape exercised in AC-1 through AC-12.
16. **AC-16 (the section's primary acceptance criterion):** A full `/develop-feature` run on a seeded feature in this repository, with all 7 guards enabled at the `standard` profile and no escape sentinels set, reaches `MERGE READY` with zero human interventions — recorded as a scripted, repeatable test result (FR-11.2), not a one-off observation.
17. **AC-17:** Post-feature hook handler count is exactly 10 (`ls hooks/handlers/*.js | wc -l` including Section 7's 3 plus this feature's 7 — `pre:edit:read-guard`'s dual registration is one handler file, not two), verifiable against the v4.0 budget of ≤12 (NFR-4).
...
| `hooks/handlers/pre-edit-read-guard.js` | `pre:edit:read-guard` — both halves (`PostToolUse` `Read` recorder, `PreToolUse` `Edit\|Write` gate), dispatched on `input.hook_event_name` (FR-4) |
| `hooks/lib/read-tracker.js` | Session-keyed "read this session" record, reusing `hooks/lib/accumulator.js`'s directory/sanitization/GC pattern for a second, independent record type (FR-4.2) |
| `hooks/handlers/pre-edit-config-protection.js` | `pre:edit:config-protection` — config-weakening and `@ts-nocheck`/blanket-`eslint-disable` checks (FR-5) |
...
| `hooks/handlers/pre-edit-gateguard.js` | `pre:edit:gateguard` — opt-in fact-forcing first-edit-per-file-per-slice gate (FR-8) |
| `hooks/lib/gateguard-state.js` | Session+slice-keyed "already fact-forced this file" record, same accumulator pattern as `read-tracker.js` (FR-8.2) |
| `tests/fixtures/hooks/guards/*` | Crafted stdin fixtures for every guard's positive (denied), negative (allowed), escape-sentinel, and fail-open cases (FR-9, FR-11) |

*(Exact handler and lib filenames are illustrative and may be finalized by the planner during `/bootstrap-feature`, per Section 7's own precedent; `hooks/hooks.json` id strings in FR-2 through FR-8 are fixed.)*

#### Modified Files

| File | Changes | Related Requirements |
|------|---------|---------------------|
| `hooks/hooks.json` | Add `PreToolUse` entries matched `Bash` (git-guard), `Write` (shrink-guard), and `Edit\|Write` (read-guard's gate half, config-protection, isolation-guard, gateguard); add a `PostToolUse` entry matched `Read` (read-guard's recorder half, same id as its `PreToolUse` entry); add a second entry to the existing `Stop` array (changelog-guard, alongside Section 7's `stop:typecheck-format`) | FR-2 through FR-8 |
| `hooks/lib/run-hook.js` | `HOOKS` map gains 7 entries (id → handler file, `timeoutMs`, `profiles`) per FR-1.5; no change to the pre-gate ES5 code path's syntax requirements | FR-1.5, FR-2 through FR-8, NFR-2 |
| `scripts/ci/validate-hooks.js` | Gains the FR-10.2 positive-control check | FR-10.2 |
| `tests/hooks/test-wrapper.js` | Narrows its static no-`exit(2)`/no-`permissionDecision` sweep to Section 7's 4 files only, per FR-10.1 | FR-10.1 |
| `.github/workflows/ci.yml` | Extend `validate-assets` to run the new guard fixture tests and the FR-10.1/FR-10.2 assertions, alongside Section 7's existing invocations | FR-10.3 |
...
| `.gitignore` (repo root), `templates/.gitignore` | `.claude/tmp/` is already covered by Section 7 FR-6.5; the new read-tracker and gateguard-state record files land in the same already-ignored directory, requiring no additional entry |
...
None. This project has no database of any kind; nothing in this feature introduces one. The session-keyed state files this feature adds (`hooks/lib/read-tracker.js`, `hooks/lib/gateguard-state.js`) are plain-text records under `.claude/tmp/`, not a schema.
...
1. **Risk: A guard wedging an unattended run (the top risk).** Any of the 7 guards denying a call the model cannot self-resolve would stall the pipeline exactly where the autonomy contract forbids. Mitigation: every guard has a scoped escape sentinel plus a `SDLC_DISABLED_HOOKS` entry (FR-9.3, FR-9.4), every deny reason states a concrete remedy classifiable by the existing deviation rules (FR-9.5, FR-9.6), `stop:changelog-guard`'s Stop-blocking is explicitly bounded rather than able to loop (FR-7.5), and the autonomy regression test (FR-11, AC-16) is the section's primary, hard acceptance gate — a feature that fails it is not done regardless of how correct each guard's individual logic is.
2. **Risk: False positives on legitimate work, especially `config-protection`.** A slice that deliberately, correctly relaxes a lint rule as part of its actual scope would otherwise be blocked indistinguishably from an unattended run cheating a red build green. Mitigation: FR-5.6's escape (`SDLC_ALLOW_CONFIG_EDIT=1`) plus FR-9.5's concrete-remedy requirement means the guard's own message tells the model exactly how to proceed; Gate 2 (Code Review, FR-5.8's backstop) would still catch a wrongly-bypassed weakening at merge.
...
4. **Risk: GateGuard's cost/benefit is unmeasured and it is deliberately the highest-variance guard in this feature.** Fact-forcing could meaningfully reduce downstream defects (the ECC rationale this feature borrows), or it could add pure friction with no measurable benefit. Mitigation: ships opt-in (FR-8.1/FR-8.5), promotion criteria are fixed in advance and falsifiable rather than a subjective call (FR-8.6), reviewed at the roadmap's F5 kickoff, and removed outright if unmeasured by then rather than left permanently opt-in-and-forgotten.
...
6. **Risk: `stop:changelog-guard`'s Stop-event blocking semantics are less familiar than `PreToolUse` deny and could be mis-implemented as if it could prevent the write that already happened.** Mitigation: FR-7.4 states precisely what "blocking" means for this hook (forcing another turn, not preventing a completed write), and FR-7.5's 2-block cap bounds worst-case behavior even if the shape check itself has a bug that never resolves.
7. **Risk: `pre:edit:read-guard`'s dual-event registration (one id, two `hooks.json` entries) is an architecture pattern Section 7 never needed.** A miswired registration (e.g., the `PostToolUse` recorder half accidentally matched on the wrong tool, or the two halves dispatching to different handler files) would make the guard either silently useless (never records, so every edit passes) or permanently blocking (never sees a matching record). Mitigation: FR-10.2's positive-control check and the FR-9 fixture requirement both exercise the dual registration explicitly, and AC-8 requires a fixture proving the record-then-gate sequence actually works end to end, not just that each half parses.
8. **Risk: Asset budget headroom.** This feature alone consumes 7 of the remaining 9 hook slots under the v4.0 cap of 12 (Section 6 NFR-5), leaving only 2 for the roadmap's F3/F4/F5. Mitigation: none of F3/F4/F5 currently plans a new top-level hook beyond folding new state into `session:start:spine`'s existing injection (the roadmap's F5 description); flagged here as a standing constraint on future roadmap features, not a defect in this one.
9. **Dependency: Section 7 (Hook Infrastructure and Non-Blocking Hooks) must exist first.** Every guard in FR-2 through FR-8 is dispatched through `hooks/lib/run-hook.js` and subject to its fail-open contract (FR-1.2); none of this feature's requirements is independently invokable without Section 7's wrapper, configuration file, and runtime controls already in place. Per the roadmap's stated execution order (F1 → F2a → F2b), this feature is F2b and lands immediately after Section 7.
10. **Dependency: Section 1 FR-2 (deviation rules).** Every guard's deny/block reason is designed to be classifiable by the existing four deviation rules (FR-9.6); a future material change to `src/rules/error-recovery.md`'s rule definitions should prompt a review of this feature's guard message wording.
11. **Dependency: Section 5 (Changelog Automation).** `stop:changelog-guard`'s shape assertions (FR-7.2) and its backstop (FR-7.6) both depend on `src/rules/changelog.md`'s format being unchanged; `pre:agent:isolation-guard`'s first backstop (FR-6.7) is the same idempotency guard Section 5 FR-1.6 defines.
