...
**Who reads it, who writes it.** Reads: `session:start:spine` injects up to 6 `Prevention Rules` entries with `Confidence: ≥0.7` into the next session's context, framed explicitly as untrusted data describing a pattern to watch for, never an instruction to execute; `planner`, during `/bootstrap-feature` Step 5, reads all `Prevention Rules` unfiltered by confidence (capped at the top 20) and attaches matching rules to slices via a `Prevention:` sub-field, without gaining `Write`/`Edit`. Writes: there are exactly **four** writing contexts, and every one of them is an orchestrator — never a subagent. (1) `/implement-slice` writes Triggers 1-2 when it runs standalone. (2) `/develop-feature`'s post-wave collection writes Triggers 2-3 from the `(category, count)` pairs its subagents report back, since a subagent can neither see its siblings' fires nor write the store itself. (3) `/bootstrap-feature` Step 5, immediately after `planner` returns, writes the `Last confirmed at` / `Retires at` confirmation for every Prevention Rule actually attached to a slice. (4) `/merge-ready` writes Trigger 3 at Post-Gate Instinct Capture and performs Consolidate Instincts. Parallel-wave subagents are refused direct writes by `pre:agent:isolation-guard`, the same mechanical backstop already protecting the scratchpad and changelog — which is precisely why contexts (2) and (3) exist as orchestrator writes rather than subagent ones.
