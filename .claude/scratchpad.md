...
Deviation rule fires this feature: rule1=3 rule2=17 rule3=1 rule4=0
(rule1 #3, post-release: stop:gate-evidence FALSE-REFUSED the MERGE READY report — its same-session filter found zero records because background-dispatched gate agents write SubagentStop wave-records under their OWN session_ids (verified: records exist in .claude/debug/wave-results/ carrying 8fde1963…/absent ids, not this orchestrator's). Handled via the guard's documented SDLC_ALLOW_UNEVIDENCED_GATES=1 override with the which-applies statement; free.)
Follow-up (guard correctness, from the false refusal above): stop:gate-evidence must recognize background/async Agent dispatch — either match wave-records by parent/orchestrator session, or accept transcript-level Agent tool_use evidence. This is the guard's own documented nightmare ("a guard that fires wrongly on honest work gets switched off"); fix next feature, with a seeded fixture reproducing the async-dispatch shape.
